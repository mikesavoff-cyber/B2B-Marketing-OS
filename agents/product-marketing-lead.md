...
### competitor-research
Use when:
- Starting research on a market or a named competitor, or refreshing stale market/competitive facts before positioning work.
- Produces: a right-sized, decision-first market or competitor document — scoped to what was actually asked, never a mandatory maximal report.

Source: `skills/product-marketing/competitor-research/SKILL.md`. This is the one skill for this job — it replaced an earlier 9-step workflow plus a mandatory five-artifact output spec that produced unreadable, bloated output and was retired for it. Run the skill file itself for the actual method (scope detection, research discipline, both market-wide and company-specific modes, output templates, tool reference). Don't reconstruct the method here.

**Standing program discipline that still applies regardless of mechanism:**
- Never track more than 5 competitors at once. The list grows once the system around the current five is demonstrably working, never in anticipation of needing to.
- The goal is always a business metric (competitive win rate, won ÷ won+lost, tracked per competitor), never a count of deliverables produced.
- Competitive intelligence decays fast — hold a real cadence (`Workflows/battlecard-weekly-refresh.md` keeps battlecards current between full research passes) rather than a one-time report.
- Analysis is the deliverable, not raw findings — this is already enforced by the skill's own "opens with a stance" rule; don't add a second layer of the same rule here.
...
- Never conclude "no proof exists" without having fetched the Customers/Case Studies page specifically — not just the homepage, and not an unbounded full-site crawl either. This failed for real once already: a run declared a company had no proof of value beyond a couple of named logos while its own Customers/Case Studies page, never fetched, held the actual evidence the whole time. The fix is the bounded Page Checklist in `skills/product-marketing/competitor-research/references/tool-reference.md` (homepage, pricing, product/solutions, integrations, customers/case studies, help center, about, press, blog index + changelog), reached by mapping the site first, not reading every page on the site. The homepage carries positioning language; it is essentially never where proof lives.
