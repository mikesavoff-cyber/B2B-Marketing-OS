...
**Directory layout.** Everything below is a *working folder*, kept outside the repo at
`~/competitor-research-work/<company>-<YYYY-MM-DD>/`, never in the project. The only file that lands in
the project is the Final Research Output (see the end of this file).

```
~/competitor-research-work/<company>-<YYYY-MM-DD>/
├── raw/
...
*Added 2026-09-23 after the Skillvue run: the working folder held 31 files, and the output that was
actually useful was one file merged from them.*

When research is done, merge everything in the working folder into **one file**:
`competitor-profiles/<company>-competitive-research-<YYYY-MM-DD>.md`. This is the only file the run
writes to the project. Build it from the working files, not from memory. Structure, in order:

1. **Header** — date, audience, tier, competitors tracked, and the source classes stated once.
2. **Summary** — the bet (one paragraph), comparison table (baseline + every competitor), positioning
   map, key takeaways, gaps and opportunities, findability comparison.
3. **Baseline profile** — the named company, condensed, ending with "How competitors would read us."
4. **One section per competitor** — condensed: URL / founded / HQ / funding line, positioning,
   pricing, reviews, the standout finding, and "Read against [company]."
5. **Sources** — pages read per company, what was blocked or not collected, and substitutes used.

Condense, don't paste: each competitor section is a short read, not the full template. Leave the
working folder where it is (outside the project) so the evidence can be re-checked later.
